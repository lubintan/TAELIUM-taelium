...
//            }
    ...
    private void trace(Block block) {
        for (Transaction transaction : block.getTransactions()) {
            ...
            if (transaction.getAmountNQT().compareTo(BigInteger.ZERO) > 0 && recipientId == 0) {
...
//    }
    ...
    private Map<String,String> getValues(long accountId, Transaction transaction, boolean isRecipient, boolean logFee, boolean logAmount) {
        ...
        BigInteger fee = transaction.getFeeNQT();
        if (isRecipient) {
            fee = BigInteger.ZERO; // fee doesn't affect recipient account
        } else {
            // for sender the amounts are subtracted
            amount = amount.negate();
            fee = fee.negate();
        }
        if (fee.compareTo(BigInteger.ZERO) == 0 && amount.compareTo(BigInteger.ZERO) == 0) {
    ...
    private Map<String,String> getValues(long accountId, Block block) {
    	BigInteger fee = block.getTotalFeeNQT();
        if (fee.compareTo(BigInteger.ZERO) == 0) {
            return Collections.emptyMap();
        }
        BigInteger totalBackFees = BigInteger.ZERO;
        if (block.getHeight() > 3) {
        	BigInteger[] backFees = new BigInteger[3];
            for (Transaction transaction : block.getTransactions()) {
            	BigInteger[] fees = ((TransactionImpl)transaction).getBackFees();
                for (int i = 0; i < fees.length; i++) {
                    backFees[i] = backFees[i].add(fees[i]);
                }
            }
            for (int i = 0; i < backFees.length; i++) {
                if (backFees[i].compareTo(BigInteger.ZERO) == 0) {
                    break;
                }
        ...
        map.put("generation fee", fee.subtract(totalBackFees).toString());
        map.put("block", block.getStringId());
        map.put("event", "block");
        map.put("timestamp", String.valueOf(block.getTimestamp()));
        map.put("height", String.valueOf(block.getHeight()));
