...
package nxt;
//seen.
//mainly for messages. and public key announcement. (which i guess is also a message.) 
...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return Taelium.getEpochTime() - transaction.getTimestamp() <
                    (includeExpiredPrunable && Constants.INCLUDE_EXPIRED_PRUNABLE ?
                            Constants.MAX_PRUNABLE_LIFETIME : Constants.MIN_PRUNABLE_LIFETIME);
        }//load prunable data if current time minus tx time, is still within the prunable lifetime.
        // if beyond, then this will return false.
    }
...
//            return isPhasable() && transaction.getPhasing() != null;
    ...
    class Message extends AbstractAppendix {
        ...
        static Message parse(JSONObject attachmentData) {
            ...
            return new Message(attachmentData); //see below.
        }

        private static final Fee MESSAGE_FEE = new Fee.SizeBasedFee(BigInteger.ZERO, Constants.ONE_TAEL, 32) {
            @Override
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_TAEL.divide(BigInteger.TEN)) {
            @Override
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            ...
            if (msg == null && Taelium.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
        ...
        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (Taelium.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    // PrunablePlainMessage class ends.
    
    abstract class AbstractEncryptedMessage extends AbstractAppendix {

        private static final Fee ENCRYPTED_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_TAEL, Constants.ONE_TAEL, 32) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendage) {
        ...
        private AbstractEncryptedMessage(EncryptedData encryptedData, boolean isText, boolean isCompressed) {
            super(isCompressed ? 1 : 2);  //calls AbstractAppendix(1) or AbstractAppendix(2)
...
//    AbstractEncryptedMessage class ends.    
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_TAEL.divide(BigInteger.TEN)) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws NxtException.ValidationException {
            ...
            if (ed == null && Taelium.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
...
//    PrunableEncryptedMessage class ends.
    ...
    } //UnencryptedPrunableEncryptedMessageEnds
...
// UnencryptedEncryptedMessage class ends.
...
//UnencryptedEncryptToSelfMessage class ends.
...
//PublicKeyAnnouncement class ends.
...
//                fee = fee.add(BigInteger.valueOf(20).multiply(Constants.ONE_TAEL));
//            } else {
//                fee = fee.add(Constants.ONE_TAEL);
//            }
//            if (phasing.hashedSecret.length > 0) {
//            	fee = fee.add(BigInteger.valueOf(1+(phasing.hashedSecret.length - 1) / 32).multiply(Constants.ONE_TAEL));
////                fee += (1 + (phasing.hashedSecret.length - 1) / 32) * Constants.ONE_TAEL;
//            }
//            fee = fee.add(Constants.ONE_TAEL.multiply(BigInteger.valueOf(phasing.linkedFullHashes.length)));
////            fee += Constants.ONE_TAEL * phasing.linkedFullHashes.length;
//            return fee;
//        };
//
