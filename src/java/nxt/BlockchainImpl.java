...
final class BlockchainImpl implements Blockchain {
    ...
    static BlockchainImpl getInstance() {
        return instance; 
    } // singleton. BlockchainImpl must be called with BlockchainImpl.getInstance(), and then <.something>.
    ...
    @Override
    public BlockImpl getECBlock(int timestamp) { //EC block is the -720th block?
...
//                buf.append(" LEFT JOIN phasing_poll_result ON transaction.id = phasing_poll_result.id ");
            ...
            int prunableExpiration = Math.max(0, Constants.INCLUDE_EXPIRED_PRUNABLE && includeExpiredPrunable ?
                                        Taelium.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME :
                                        Taelium.getEpochTime() - Constants.MIN_PRUNABLE_LIFETIME);
            if (withMessage) {
